...
package com.manmanbuy.mbaichuan;
...
public class RNReactNativeMbaichuanModule extends ReactContextBaseJavaModule {
    ...
    public void show(final ReadableMap param, final Promise p) {
        ...
        switch (type) {
            case "detail":
                this._show(new AlibcDetailPage(payload.getString("itemid")), param, p);
                break;
            case "url":
                this._show(new AlibcPage(payload.getString("url")), param, p);
                break;
            case "shop":
                this._show(new AlibcShopPage(payload.getString("shopid")), param, p);
                break;
            case "orders":
                this._show(new AlibcMyOrdersPage(payload.getInt("orderStatus"), payload.getBoolean("allOrder")), param, p);
                break;
            case "addCard":
                this._show(new AlibcAddCartPage(param.getString("itemid")), param, p);
                break;
            case "mycard":
                this._show(new AlibcMyCartsPage(), param, p);
                break;
            default:
                WritableMap map = Arguments.createMap();
        ...
        AlibcTrade.show(getCurrentActivity(),
                ...
                new AlibcTradeCallback() {
                    public void onTradeSuccess(AlibcTradeResult tradeResult) {
                        ...
                        WritableMap map = Arguments.createMap();
                        map.putString("orderid", "" + tradeResult.payResult.paySuccessOrders);
                        map.putString("message", "success");
                        map.putString("code", "0");
                        reactContext.getJSModule(RCTEventEmitter.class).receiveEvent(
                                webview.getId(),
    ...
    private void _show(AlibcBasePage page, final ReadableMap param, final Promise p) {
        // 处理参数
        AlibcShowParams showParams = new AlibcShowParams();
        showParams = this.dealShowParams(param);
        AlibcTaokeParams taokeParams = new AlibcTaokeParams();
    ...
    private Map<String, String> dealExParams(final ReadableMap param) {
        ...
        String isvcode = "app";
        // 设置tkkey
        if (payload.hasKey("tkkey") && (payload.getString("isvcode") != null
